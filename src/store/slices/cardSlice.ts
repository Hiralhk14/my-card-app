...
const cardSlice = createSlice({
  ...
  reducers: {
    ...
    addCard(state, action: PayloadAction<Card>) {
      ...
      const typeCards = state.cards.filter(c => c.cardType === action.payload.cardType);
    ...
    lockCard(state, action: PayloadAction<string>) {
      ...
      if (card) {
        ...
        if (card.isLocked) clearOtherCardFlags(card, "isLocked");
        saveCardsToStorage(state.cards);
      }
    },
    ...
    archiveCard(state, action: PayloadAction<string>) {
      ...
      if (card) {
        ...
        if (card.isArchived) clearOtherCardFlags(card, "isArchived");
        saveCardsToStorage(state.cards);
      }
    },

    setDefaultCard(state, action: PayloadAction<{ id: string; cardType: CardType }>) {
    ...
    toggleDefaultCard(state, action: PayloadAction<{ id: string; cardType: CardType }>) {
      ...
      state.cards.forEach(c => {
        ...
        if (c.id === action.payload.id) {
          ...
          if (c.isDefault) clearOtherCardFlags(c, "isDefault");
        } else {
          c.isDefault = false;
        }
      });
    ...
    toggleGPay(state, action: PayloadAction<string>) {
      ...
      if (card) {
        ...
        if (card.addToGPay) clearOtherCardFlags(card, "addToGPay");
        saveCardsToStorage(state.cards);
      }
    },

    setActiveCardIndex(state, action: PayloadAction<{ cardType: CardType; index: number }>) {
